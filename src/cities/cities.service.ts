...
export class CitiesService {
  ...
  findAll(keyword: string = "", province_id: number = null, fields: string[] = null): Promise<City[]> {
    const py = { where: null, relations: ['province', 'province.country'] };
    ...
    if (province_id != null) {
      py['where'].push({ province: { id: province_id } });
    }
    return this.citiesRepository.find(py);
  }

  findOne(id: number): Promise<City> {
    return this.citiesRepository.findOne({ where: { id: id }, relations: ['province'] });
  }
